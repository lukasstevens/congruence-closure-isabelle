...
proof -
  ...
  from eff_union assms(1-3) show lca_eq:
    ...
    by (simp add: ufe_ds'_def uf_ds_ufe_union_eq)
...
lemma explain'_dom_if_ufe_rep_of_eq:
  ...
  using assms
proof(induction ufe_ds arbitrary: x y rule: ufe_induct)
  case (init ufe x y)
  then have "x = y"
...
next
  ...
  from ufe_union have in_Field_ufe_\<alpha>:
    "x \<in> Field (ufe_\<alpha> ufe_ds)" "y \<in> Field (ufe_\<alpha> ufe_ds)"
    by simp_all
  from ufe_union.hyps this ufe_union.prems(3) show ?case
  proof(cases rule: ufe_rep_of_ufe_union_eq_cases)
    case 1
    with ufe_union have "explain'_dom ufe_ds (x, y)"
      by simp
    with 1 ufe_union.prems ufe_union.hyps show ?thesis
      by (intro explain'_dom_ufe_union in_Field_ufe_\<alpha> ufe_union.hyps) blast
  next
    case 2
    note find_newest_on_path_ufe_union[OF ufe_union.hyps px]
    with 2 ufe_union.prems ufe_union.hyps have newest_x_eq:
      ...
      using ufa_lca_ufa_union[OF ufe_union.hyps in_Field_ufe_\<alpha>]
  ...
  next
    case 3
    note find_newest_on_path_ufe_union[OF ufe_union.hyps py]
...
next
  ...
  moreover from newest_x.hyps have
    ...
    by force
  moreover from newest_x have "explain'_dom (ufe_union ufe_ds a b) (x, y)"
...
next
  ...
  moreover from newest_y have
    ...
    find_newest_on_path ufe_ds (ufe_lca ufe_ds x y) x"
    by order
  moreover from newest_y have "explain'_dom (ufe_union ufe_ds a b) (x, y)"
    by (intro explain'_dom_if_ufe_rep_of_eq)
      (simp_all add: uf_ds_ufe_union_eq ufa_rep_of_ufa_union)
  moreover note
    find_newest_on_path_ufe_union_ufe_lca
...
lemma explain_eq_explain':
  assumes "x \<in> Field (ufe_\<alpha> ufe_ds)" "y \<in> Field (ufe_\<alpha> ufe_ds)"
  assumes "ufe_rep_of ufe_ds x = ufe_rep_of ufe_ds y"
  shows "explain ufe_ds x y = explain' ufe_ds x y"
  using assms                                                 
...
next
  ...
  proof(cases rule: ufe_rep_of_ufe_union_eq_cases)
    case 1
    with ufe_union have
      "explain (ufe_union ufe_ds a b) x y = explain ufe_ds x y"
  ...
  next
    ...
    from 2 have "explain (ufe_union ufe_ds a b) x y =
    ...
    also from 2 ufe_union.hyps have "\<dots> =
      ...
      using in_Field_ufe_\<alpha> \<open>eff_union (uf_ds ufe_ds) a b\<close>
    ...
    proof -
      ...
      "ufe_lca (ufe_union ufe_ds a b) x y = ufe_rep_of ufe_ds b"
        ...
        using ufa_lca_ufa_union \<open>eff_union (uf_ds ufe_ds) a b\<close> by metis
      moreover from this 2 in_Field_ufe_\<alpha> have
        "?newest_x = Some (length (unions ufe_ds))"
        using find_newest_on_path_ufe_union[OF \<open>eff_union (uf_ds ufe_ds) a b\<close> px]
  ...
  next
    ...
    from 3 have "explain (ufe_union ufe_ds a b) x y =
    ...
    proof -
      ...
      moreover from this 3 in_Field_ufe_\<alpha> have
        "?newest_y = Some (length (unions ufe_ds))"
        using find_newest_on_path_ufe_union[OF \<open>eff_union (uf_ds ufe_ds) a b\<close> py]
        by (metis ufa_rep_of_ufa_rep_of)
