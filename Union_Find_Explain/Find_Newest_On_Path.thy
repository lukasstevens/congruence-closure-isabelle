...
proof -
  from assms have "y \<in> verts (ufe_forest_of ufe_ds)"
    by blast
...
lemma find_newest_on_path_ufe_union_if_ufe_rep_of_neq:
  ...
  assumes "pre_digraph.awalk (ufe_forest_of (ufe_union ufe_ds a b)) x pxy y"
  shows "find_newest_on_path (ufe_union ufe_ds a b) x y = Some (length (unions ufe_ds))"
proof -
  interpret ufe_forest where ufe_ds = ufe_ds .
  interpret ufe_union_forest: ufe_forest
    where ufe_ds = "ufe_union ufe_ds a b" .
  from assms ufa_union_awalk_eq have
    "x = ufe_rep_of ufe_ds b"
    "pxy = (ufe_rep_of ufe_ds b, ufe_rep_of ufe_ds a) # awalk_from_rep (uf_ds ufe_ds) y"
    using uf_ds_ufe_union_eq by metis+
  moreover from assms have
    "awalk (ufe_rep_of ufe_ds y) (awalk_from_rep (uf_ds ufe_ds) y) y"
  ...
  then have set_awalk_from_rep_subs_arcs:
    "set (awalk_from_rep (uf_ds ufe_ds) y) \<subseteq> arcs (ufe_forest_of ufe_ds)"
    by blast
  then have
    "ufe_union_forest.au_of ` set (awalk_from_rep (uf_ds ufe_ds) y) =
      au_of ` set (awalk_from_rep (uf_ds ufe_ds) y)"
    using ufe_union_au_of_eq_if_in_arcs[OF assms(1)] by force
  moreover have
    "ufe_union_forest.au_of (ufe_rep_of ufe_ds b, ufe_rep_of ufe_ds a) = length (unions ufe_ds)"
    by (subst ufe_union_au_of_eq_if_head_eq[OF assms(1)]) simp_all
  ultimately have "ufe_union_forest.au_of ` set pxy =
    insert (length (unions ufe_ds)) (au_of ` set (awalk_from_rep (uf_ds ufe_ds) y))"
    by simp
  moreover have
    "\<forall>i \<in> au_of ` set (awalk_from_rep (uf_ds ufe_ds) y). i \<le> length (unions ufe_ds)"
    using set_awalk_from_rep_subs_arcs au_of_lt_length_unions
    by (simp add: in_mono order_less_imp_le)
  ultimately have "Max (ufe_union_forest.au_of ` set pxy) = length (unions ufe_ds)"
...
proof -
  ...
  consider
    (ufe_rep_of_neq) "ufe_rep_of ufe_ds x \<noteq> ufe_rep_of ufe_ds y" |
    (eq) "x = y" |
    (ufe_rep_of_eq) "x \<noteq> y" "ufe_rep_of ufe_ds x = ufe_rep_of ufe_ds y"
    by auto
  then show ?thesis
  proof cases
    case eq
  ...
  next
    ...
    then have pxy_subs_arcs: "set pxy \<subseteq> arcs (ufe_forest_of ufe_ds)"
      by blast
    ...
    also have "\<dots> = find_newest_on_path ufe_ds x y"
...
lemma find_newest_on_path_ufe_union_if_reachable:
  assumes "eff_union (uf_ds ufe_ds) a b"
  assumes "x \<rightarrow>\<^sup>*\<^bsub>ufe_forest_of (ufe_union ufe_ds a b)\<^esub> y"
  shows "find_newest_on_path (ufe_union ufe_ds a b) x y = 
