...
lemma lookup_au_ds_lt_length_unions:
  "au_ds ufe x = Some i \<Longrightarrow> i < length (unions ufe)"
proof(induction ufe arbitrary: x i rule: ufe_induct)
...
lemma dom_au_ds_subs_Field_ufa_\<alpha>:
  "dom (au_ds ufe) \<subseteq> Field (ufe_\<alpha> ufe)"
proof(induction ufe rule: ufe_induct)
...
lemma ufe_rep_of_eq_if_au:
  assumes "au_ds ufe x = Some i"
  assumes "unions ufe ! i = (a, b)"
  shows
    "ufe_rep_of ufe a = ufe_rep_of ufe x"
    "ufe_rep_of ufe b = ufe_rep_of ufe x"
proof -
  have
    "ufe_rep_of ufe a = ufe_rep_of ufe x \<and>
    ufe_rep_of ufe b = ufe_rep_of ufe x"
    using assms
  proof(induction ufe arbitrary: i a b rule: ufe_induct)
    case (ufe_union ufe y z)

    from ufe_union.prems have "(a, b) \<in> set (unions (ufe_union ufe y z))"
      by (metis lookup_au_ds_lt_length_unions nth_mem)
    with ufe_union.hyps have
      a_in_Field_\<alpha>: "a \<in> Field (ufe_\<alpha> ufe)" and
      b_in_Field_\<alpha>: "b \<in> Field (ufe_\<alpha> ufe)"
      using Field_unions_subs_Field_ufe_\<alpha>
      by (auto intro: FieldI1 FieldI2)
    from ufe_union have x_in_Field_\<alpha>: "x \<in> Field (ufe_\<alpha> ufe)"
    ...
    proof(cases "i < length (unions ufe)")
      ...
      with ufe_union.hyps False have "i = length (unions ufe)"
        by simp
      moreover from calculation have "au_ds ufe x \<noteq> Some i"
      ...
      next
      case True
      with ufe_union.prems ufe_union.hyps have "au_ds ufe x = Some i"
        by (simp split: if_splits)
      moreover from ufe_union.hyps True have "unions (ufe_union ufe y z) ! i = unions ufe ! i"
...
lemma lookup_au_ds_eq_None_iff:
  assumes "z \<in> Field (ufe_\<alpha> ufe)"
  shows "au_ds ufe z \<noteq> None \<longleftrightarrow> ufe_rep_of ufe z \<noteq> z"
  using assms
...
lemma inj_au_ds:
  shows "inj_on (au_ds ufe) (dom (au_ds ufe))"
proof(induction ufe rule: ufe_induct)
  case init
  then show ?case
...
definition "au_of a \<equiv> the (au_ds ufe (head (ufe_forest_of ufe) a))"

sublocale ufa_forest where uf = "uf_ds ufe"
  by unfold_locales
...
interpretation ufe_union_forest: ufe_forest where
  ufe = "ufe_union ufe a b"
  by unfold_locales

lemma in_verts_ufe_forest_of_ufe_union_if_in_verts[simp, intro]:
  assumes "x \<in> verts (ufe_forest_of ufe)"
  shows "x \<in> verts (ufe_forest_of (ufe_union ufe a b))"
  using assms eff_union
  unfolding verts_ufa_forest_of by auto

lemma in_arcs_ufe_forest_of_ufe_union_if_in_arcs[simp, intro]:
  assumes "e \<in> arcs (ufe_forest_of ufe)"
  shows "e \<in> arcs (ufe_forest_of (ufe_union ufe a b))"
  using assms eff_union
...
proof -
  ...
  with assms eff_unionD[OF eff_union] have
    "head (ufe_forest_of ufe) e \<noteq> ufe_rep_of ufe a"
...
lemma ufe_union_au_of_eq_if_head_eq:
  assumes "head (ufe_forest_of (ufe_union ufe a b)) e = ufe_rep_of ufe a"
  shows "ufe_union_forest.au_of e = length (unions ufe)"
...
lemma head_in_dom_lookup_if_in_arcs:
  assumes "e \<in> arcs (ufe_forest_of ufe)"
  shows "head (ufe_forest_of ufe) e \<in> dom (au_ds ufe)"
  using assms
proof -
  from assms have
    "head (ufe_forest_of ufe) e \<in> Field (ufe_\<alpha> ufe)" (is "?a \<in> _")
    using head_in_verts by blast
  moreover from assms have "ufe_rep_of ufe ?a \<noteq> ?a"
    using arc_implies_awalk awalk_and_ufa_rep_of_reflD(1) loopfree.no_loops by blast
  ultimately obtain i where "au_ds ufe ?a = Some i"
...
lemma au_of_lt_length_unions:
  assumes "e \<in> arcs (ufe_forest_of ufe)"
  shows "au_of e < length (unions ufe)"
...
lemma ufe_rep_of_eq_au_of:
  assumes "e \<in> arcs (ufe_forest_of ufe)"
  assumes "unions ufe ! au_of e = (a, b)"
  shows
    "ufe_rep_of ufe a = ufe_rep_of ufe b"
proof -
  from head_in_dom_lookup_if_in_arcs[OF assms(1)] have
    "au_ds ufe (head (ufe_forest_of ufe) e) = Some (au_of e)"
...
lemma inj_on_au_of_arcs:
  "inj_on au_of (arcs (ufe_forest_of ufe))"
proof(intro inj_onI)
  let ?F = "ufe_forest_of ufe"
  fix y z
  assume
    "y \<in> arcs ?F"
    "z \<in> arcs ?F"
...
lemma newest_on_path_lt_length_unions:
  assumes "newest_on_path newest x p y"
  assumes "x \<noteq> y"
  shows "newest < length (unions ufe)"
proof -
...
lemma newest_on_path_valid_union:
  assumes "newest_on_path newest x p y"
  assumes "x \<noteq> y"
  assumes "unions ufe ! newest = (a, b)"
  shows "a \<in> Field (ufe_\<alpha> ufe)" "b \<in> Field (ufe_\<alpha> ufe)"
proof -
  from assms have "(a, b) \<in> set (unions ufe)"
    using newest_on_path_lt_length_unions by (metis nth_mem)
  then show "a \<in> Field (ufe_\<alpha> ufe)" "b \<in> Field (ufe_\<alpha> ufe)"
    using Field_unions_subs_Field_ufe_\<alpha>
    by (auto intro: FieldI1 FieldI2)
qed
...
proof -
  from newest_on_pathE[OF assms(1,2)] obtain i where
    ...
    by blast
  moreover from this have "i \<in> arcs (ufe_forest_of ufe)"
    by blast
  note head_in_dom_lookup_if_in_arcs[OF this]
  ultimately have "au_ds ufe (head (ufe_forest_of ufe) i) = Some newest"
    unfolding au_of_def by force
  note ufe_rep_of_eq_if_au[OF this assms(3)]
  moreover from \<open>i \<in> set p\<close> \<open>awalk x p y\<close> have
    "ufe_rep_of ufe x = ufe_rep_of ufe (head (ufe_forest_of ufe) i)"
